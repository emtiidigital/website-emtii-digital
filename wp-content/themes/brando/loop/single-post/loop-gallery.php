				...
				foreach ($brando_gallery as $key => $value) 
				{
					...
					if($brando_thumb[0]):
		                ...
		                echo '<div class="col-md-4 col-sm-6 col-xs-12 no-padding">';
		                    echo '<a title="'.get_the_title().'" href="'.esc_url($brando_thumb[0]).'" class="lightboxgalleryitem" data-group="'.esc_attr($brando_popup_id).'"><img src="'.esc_url($brando_thumb[0]).'" width="'.$brando_thumb[1].'" height="'.$brando_thumb[2].'" alt=""'.$srcset_data.'/></a>';
		                echo '</div>';
		            endif;
				}
			endif;
				...
				if(is_array($brando_gallery)):
					foreach ($brando_gallery as $key => $value) {
						...
						if($brando_thumb[0]):
				            ...
				            echo '<div class="item"><img src="'.esc_url($brando_thumb[0]).'" width="'.$brando_thumb[1].'" height="'.$brando_thumb[2].'" alt=""'.$srcset.'/></div>';
				        endif;
					}
				endif;
	        echo '</div>';
