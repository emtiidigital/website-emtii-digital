...
if($brando_blog_lightbox_gallery == 1){
	...
	if(is_array($brando_gallery)){
		foreach ($brando_gallery as $key => $value) {
			...
			$brando_thumb = wp_get_attachment_image_src( $value, $brando_srcset_data );
			if($brando_thumb[0]){
				$srcset = $srcset_data = '';
				$srcset = wp_get_attachment_image_srcset( $value, $brando_srcset_data );
				if( $srcset ){
				    $srcset_data = ' srcset="'.esc_attr( $srcset ).'"';
				}
                echo '<div class="col-md-4 col-sm-6 col-xs-12 no-padding">';
                    echo '<a class="lightboxgalleryitem" data-group="'.esc_attr($brando_popup_id).'" '.$brando_image_lightbox_title.$brando_image_lightbox_caption.' href="'.$brando_thumb[0].'"><img src="'.esc_url($brando_thumb[0]).'" width="'.$brando_thumb[1].'" height="'.$brando_thumb[2].'" '.$brando_image_alt.$brando_image_title.$srcset_data.'/></a>';
                echo '</div>';
            }
		}
	}
			...
			if(is_array($brando_gallery)){
				foreach ($brando_gallery as $key => $value) {
					...
					if($brando_thumb[0]){
			            ...
			            echo '<div class="item"><img src="'.esc_url($brando_thumb[0]).'" width="'.$brando_thumb[1].'" height="'.$brando_thumb[2].'" '.$brando_image_alt.$brando_image_title.$srcset_data.'/></div>';
					}
				}
			}
		echo '</div>';
