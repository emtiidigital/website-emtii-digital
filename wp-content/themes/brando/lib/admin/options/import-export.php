...
$brando_all_demo_setup = array(
					array(
						'theme_id' 			=> 'corporate',
						'theme_name' 		=> esc_html__('Corporate', 'brando'),
						'theme_preview_url' => esc_html__('http://wpdemos.themezaa.com/brando/corporate/', 'brando'),
						'theme_screenshot' 	=> get_template_directory_uri() . '/assets/images/home-classic-corporate.jpg',
					),
					array(
						'theme_id' 			=> 'agency',
...
foreach ( $brando_all_demo_setup as $key => $demo_setup ) {
	...
	$brando_import_export_content .= '<div class="theme">
					                <div class="theme-wrapper">
					                    ...
					                    if( class_exists( 'Brando_Addons' ) && !is_plugin_active( 'wordpress-importer/wordpress-importer.php' ) ) {
...
if( class_exists( 'Brando_Addons' ) &&  is_plugin_active( 'wordpress-importer/wordpress-importer.php' ) ){
	$brando_import_export_notice .= '<strong class="font-16px" style="color:red">'.esc_html__( 'Notice: Please deactivate WordPress Importer plugin and then try demo data import to make it run successfully.', 'brando').'</strong><br/><br/>';
}
...
$this->sections[] = array(
	...
	'fields' => array(
		...
		array(
			...
			'content'		=> '
				'.$brando_import_export_notice.'
				<div id="run-regenerate-thumbnails" '.$brando_message.'><div class="brando-importer-notice">
