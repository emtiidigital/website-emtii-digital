...
$brando_excerpt_length = (isset($brando_options['brando_blog_page_excerpt_length'])) ? $brando_options['brando_blog_page_excerpt_length'] : '';
$brando_blog_page_layout = (isset($brando_options['brando_blog_page_grid_layout'])) ? $brando_options['brando_blog_page_grid_layout'] : '';
$brando_blog_page_column = (isset($brando_options['brando_blog_page_grid_column'])) ? $brando_options['brando_blog_page_grid_column'] : '';
$brando_show_thumbnail = (isset($brando_options['brando_blog_page_show_thumbnail'])) ? $brando_options['brando_blog_page_show_thumbnail'] : '';
$brando_show_separator = (isset($brando_options['brando_blog_page_show_separator'])) ? $brando_options['brando_blog_page_show_separator'] : '';
$brando_show_post_date = (isset($brando_options['brando_blog_page_show_post_date'])) ? $brando_options['brando_blog_page_show_post_date'] : '';
$brando_show_post_author = (isset($brando_options['brando_blog_page_show_post_author'])) ? $brando_options['brando_blog_page_show_post_author'] : '';
$brando_show_excerpt = (isset($brando_options['brando_blog_page_show_excerpt'])) ? $brando_options['brando_blog_page_show_excerpt'] : '';
$brando_show_category = (isset($brando_options['brando_blog_page_show_category'])) ? $brando_options['brando_blog_page_show_category'] : '';
$brando_show_comments = (isset($brando_options['brando_blog_page_show_comments'])) ? $brando_options['brando_blog_page_show_comments'] : '';
$brando_show_pagination = (isset($brando_options['brando_blog_page_show_pagination'])) ? $brando_options['brando_blog_page_show_pagination'] : '';
$brando_show_social_icon = (isset($brando_options['brando_blog_page_show_social_icon'])) ? $brando_options['brando_blog_page_show_social_icon'] : '';
$brando_show_button = (isset($brando_options['brando_blog_page_show_button'])) ? $brando_options['brando_blog_page_show_button'] : '';
$brando_button_text = (isset($brando_options['brando_blog_page_button_text'])) ? $brando_options['brando_blog_page_button_text'] : 'Continue';
$brando_date_format = (isset($brando_options['brando_blog_page_date_format'])) ? $brando_options['brando_blog_page_date_format'] : '';
$brando_show_content = (isset($brando_options['brando_blog_page_show_content'])) ? $brando_options['brando_blog_page_show_content'] : '';
$brando_srcset_data = (isset($brando_options['brando_blog_srcset_data'])) ? $brando_options['brando_blog_srcset_data'] : '';

switch ($brando_blog_page_column) {
	case '2':
...
$brando_blog_layout = ' class="blog-'.esc_attr($brando_blog_page_column).'-column"';
if( have_posts() ){

	if( $brando_blog_page_layout == 'grid'){
    ...
    echo '<div '.$brando_post_classes.'>';
		$brando_post_cat = $brando_post_meta = array();
        ...
        foreach ($brando_categories as $k => $cat) {
            ...
            $brando_post_cat[]='<a href="'.esc_url($brando_cat_link).'" class="text-small dark-gray-text text-uppercase alt-font" rel="category tag">'.esc_attr($cat->name).'</a>';
        }
        ...
        switch ( $brando_blog_page_layout ) {
        	case 'grid':
        		echo '<div class="blog-post-style7'.esc_attr($brando_class_column).' margin-seven-bottom">';
	                ...
	                echo'<div class="post-details margin-eight-top">';
	                    echo'<span class="text-large text-uppercase display-block alt-font margin-four-bottom sm-text-medium xs-text-medium alt-font font-weight-600">';
	                        echo'<a href="'.get_permalink().'" class="dark-gray-text entry-title">'.get_the_title().'</a>';
	                        echo'</span>';
	                    if( $brando_show_excerpt == 1 ){
	                		...
	                		echo '<p class="margin-seven-bottom xs-margin-five-bottom entry-content">'.$show_content.'</p>';
	                	}
	                    ...
	                    if( $brando_show_post_date == 1 || $brando_show_post_author == 1 ){
		                    echo '<span class="text-small text-uppercase alt-font light-gray-text">';
			                    if( $brando_show_post_date == 1 ){
			                        $brando_post_meta[] = '<span class="published">'.get_the_date($brando_date_format).'</a></span><time class="updated display-none" datetime="'.esc_attr( get_the_modified_date( 'c' ) ).'">'.get_the_modified_date( $brando_date_format ).'</time>';
			                    }
			                    if( $brando_show_post_author == 1 ){
			                        $brando_post_meta[] = esc_html__('Posted by','brando').' <span class="author vcard"><a href="'.get_author_posts_url( get_the_author_meta( 'ID' ) ).'" class="light-gray-text url fn n">'.get_the_author().'</span>';
			                    }
			                    echo implode(" / ", $brando_post_meta );
		                    echo'</span>';
	                	}
	                echo'</div>';
	            echo'</div>';
        	...
        	case 'classic':
				...
				echo '<article class="blog-listing">';
			        echo '<div class="col-md-2 col-sm-2 col-xs-12 text-center alt-font overflow-hidden no-padding-left sm-no-padding md-no-padding">';
			            echo '<div class="post-date xs-margin-lr-auto xs-no-margin-top fast-yellow-text"><span>'.get_the_date("d").'</span>'.get_the_date('M Y').'</div><time class="updated display-none" datetime="'.esc_attr( get_the_modified_date( 'c' ) ).'">'.get_the_modified_date('d M Y').'</time>';
			        echo '</div>';
				    ...
				    echo '<span class="alt-font display-block title-small xs-text-extra-large sm-text-medium xs-text-center margin-four-bottom entry-title"><a href="'.get_permalink().'">'.get_the_title().'</a></span>';
				    	if( $brando_show_thumbnail == 1 ){
					    	if($brando_post_format == 'image'){
			                    get_template_part('loop/loop','image');
			                }elseif($brando_post_format == 'gallery'){
			        	...
			        	if( $brando_show_excerpt == 1 ){
	                		...
	                		$show_content = apply_filters( 'the_content', $post->post_content );
	                		echo '<div class="margin-five-tb xs-margin-three-tb entry-content">'.$show_content.'</div>';
	                	}
						...
						if( $brando_show_button == 1 ){
						    echo '<div class="col-md-12 col-sm-12 col-xs-12 no-padding sm-text-left xs-text-center margin-four-tb">';
						        echo '<a href="'.get_permalink().'" class="btn-black btn btn-very-small no-margin">'.$brando_button_text.'</a>';
						    echo '</div>';
						}
					    if( $brando_show_separator == 1 ){
    ...
    if( $brando_blog_page_layout == 'grid'){
   	...
   	wp_reset_postdata();
    if( $wp_query->max_num_pages > 1 && $brando_show_pagination == 1 ){
