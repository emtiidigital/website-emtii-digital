...
$brando_show_post_date = (isset($brando_options['brando_general_show_post_date'])) ? $brando_options['brando_general_show_post_date'] : '';
$brando_show_post_author = (isset($brando_options['brando_general_show_post_author'])) ? $brando_options['brando_general_show_post_author'] : '';
...
$brando_show_pagination = (isset($brando_options['brando_general_show_pagination'])) ? $brando_options['brando_general_show_pagination'] : '';
...
$brando_button_text = (isset($brando_options['brando_general_button_text'])) ? $brando_options['brando_general_button_text'] : esc_html__('Continue','brando');
	...
	while ( have_posts() ) : the_post();
	    echo '<div '.$brando_post_classes.'>';
	        ...
	        foreach ($brando_categories as $k => $cat) {
	            ...
	            $brando_post_cat[]='<a href="'.esc_url($brando_cat_link).'" class="text-small dark-gray-text text-uppercase font-weight-600 alt-font" rel="category tag">'.esc_attr($cat->name).'</a>';
	        }
	        ...
	        switch ( $brando_general_layout ) {
	        		...
	        		echo '<div class="blog-post-style7'.esc_attr($brando_class_column).' margin-seven-bottom">';
		                ...
		                echo '<div class="post-details margin-eight-top">';
		                    ...
		                    if( $brando_show_excerpt == 1 ){
		                    	echo '<p class="margin-seven-bottom xs-margin-five-bottom entry-content">'.$brando_show_excerpt_grid.'</p>';
		                	}
	        	...
	        	case 'classic':
					...
					echo '<article class="blog-listing">';
					    ...
					    echo '<span class="alt-font font-weight-600 display-block title-small xs-text-extra-large sm-text-medium xs-text-center margin-four-bottom entry-title"><a href="'.get_permalink().'">'.get_the_title().'</a></span>';
					    	if( $brando_show_thumbnail == 1 ){
						    	if($brando_post_format == 'image'){
				                    get_template_part('loop/loop','image');
				                }elseif($brando_post_format == 'gallery'){
				        	...
				        	if( $brando_show_excerpt == 1 ){
								echo '<div class="margin-five-tb xs-margin-three-tb entry-content">'.$brando_show_excerpts.'</div>';
							}
