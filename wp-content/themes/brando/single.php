...
if($brando_enable_title != 0){
    ...
    $brando_enable_date = ($brando_options['brando_enable_meta_date'] == 1 ) ? '<span class="published">'.esc_attr(get_the_date('d F Y')).'</span><time class="updated display-none" datetime="'.esc_attr( get_the_modified_date( 'c' ) ).'">'.get_the_modified_date( 'd F Y' ).'</time>' : '';
    $brando_enable_author = ($brando_options['brando_enable_meta_author'] == 1 ) ? esc_html__('By ','brando').'<span class="author vcard"><a class="url fn n white-text" href='.esc_url(get_author_posts_url( get_the_author_meta( 'ID' ) )).'>'.get_the_author().'</a></span>' : '';
                    ...
                    echo '<div class="col-text-middle-main">';
                        echo '<div class="col-text-middle">';
                            echo '<div class="col-md-12 col-sm-12 text-center">';
                                echo '<h2 class="alt-font white-text font-weight-600 xs-title-extra-large no-margin entry-title">'.get_the_title().'</h2>';
               ...
               echo '<div class="blog-listing">';
                    ...
                    echo '<article>';
                        ...
                        if($brando_post_format == 'image'){
                            ...
                            {
                                echo get_the_post_thumbnail( get_the_ID(), $brando_image_srcset, $brando_img_attr );
                            }
